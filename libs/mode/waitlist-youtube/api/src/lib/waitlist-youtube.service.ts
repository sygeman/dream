...
export class WaitlistYoutubeService {
  ...
  async updateWaitlistState({ waitlistId, itemId = null, duration = 0 }) {
    ...
    const waitlistYoutubeUpdated = await this.prisma.modeWaitlistYoutube.update(
      {
        where: { id: waitlistId },
        data: { itemId },
      }
  ...
  async setVideo({ channelId, manualSkip = false }) {
    ...
    if (waitlistYoutube?.itemId) {
      ...
      await this.prisma.modeWaitlistYoutubeItem.update({
    ...
    const item = await this.prisma.modeWaitlistYoutubeItem.findFirst({
      where: {
        channelId,
        startedAt: null,
        canceled: false,
    ...
    const updatedItem = await this.prisma.modeWaitlistYoutubeItem.update({
      where: { id: itemId },
  ...
  async addVideo({
    ...
    // Set track to video
    const newItem = await this.prisma.modeWaitlistYoutubeItem.create({
      data: {
    ...
    const waitlistYoutubeIsEmpty =
      await this.prisma.modeWaitlistYoutube.findFirst({
